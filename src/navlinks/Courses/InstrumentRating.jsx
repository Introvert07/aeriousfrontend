...
const InstrumentRating = () => {
  ...
  const handleFormSubmit = (e) => {
    ...
    const subject = encodeURIComponent(`Instrument Rating Enquiry - ${formData.name}`);
    
    const body = encodeURIComponent(
      `Dear Aerius Team,\n\n` +
      `I am interested in enrolling for the Instrument Rating (IR) program.\n\n` +
      `--- CANDIDATE INFO ---\n` +
      `Name: ${formData.name}\n` +
          ...
          <div className="lg:col-span-4 mt-8 lg:mt-0">
            <div className="lg:sticky lg:top-10 bg-white p-6 md:p-8 rounded-[2rem] shadow-2xl border-t-8 border-[#e21d1d]">
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                ...
                ></textarea>
                <button type="submit" className="w-full bg-[#1a2e6e] text-white font-black py-4 md:py-5 rounded-2xl uppercase tracking-widest hover:bg-[#e21d1d] transition-colors flex items-center justify-center gap-3 text-xs md:text-sm">
                  Submit <Send size={16} />
                </button>
              </form>
            </div>
